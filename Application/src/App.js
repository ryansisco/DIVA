...
onSelect = (option, e) => {
	this.setState({
		...
		filteredRows: {
			...this.state.rows,
			[option]: e.value /* getRows(this.state.file, this.state.axes.option)*/
		}
			/* xRows: getRows(this.state.file, this.state.axes.x),
				yRows: getRows(this.state.file, this.state.axes.y),
				zRows: getRows(this.state.file, this.state.axes.z) */
			})
}
